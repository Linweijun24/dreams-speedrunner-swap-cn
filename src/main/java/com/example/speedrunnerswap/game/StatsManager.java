...
public class StatsManager {
    ...
    public void displayStats() {
        ...
        Msg.broadcast("\n§6=== 游戏统计 ===");
        Msg.broadcast("§7总游戏时间: §f" + formatTime(gameDuration));
        for (java.util.Map.Entry<java.util.UUID, PlayerStats> e : playerStats.entrySet()) {
            Player p = Bukkit.getPlayer(e.getKey());
            if (p == null) continue;
            PlayerStats s = e.getValue();
            Msg.broadcast("§e" + p.getName() + " §7- 击杀: §f" + s.getKills() +
                    " §7死亡: §f" + s.getDeaths() +
                    " §7活跃时间: §f" + formatTime(s.getActiveTime()) +
                    " §7移动距离: §f" + String.format(java.util.Locale.ROOT, "%.1f", s.getDistanceTraveled()) + "米");
        }
    }
