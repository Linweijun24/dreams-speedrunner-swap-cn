...
public class GameManager {
        ...
        if (!canStartGame()) {
            ...
            String failureMessage = switch (mode) {
                case DREAM -> hasRunner
                        ? "§c游戏无法开始：追猎交换模式需要至少分配一名猎人。"
                        : "§c游戏无法开始：请至少分配一名速通者。";
                case SAPNAP -> hasRunner
                        ? (hasHunter
                                ? "§c游戏无法开始：速通者交换模式不允许猎人存在。请清除猎人并只保留速通者。"
                                : "§c游戏无法开始：请至少分配一名速通者。")
                        : "§c游戏无法开始：请至少分配一名速通者。";
                case TASK -> hasRunner
                        ? (hasHunter
                                ? "§c游戏无法开始：任务大师模式只使用速通者。请在开始前移除所有猎人。"
                                : "§c游戏无法开始：请至少分配一名速通者。")
                        : "§c游戏无法开始：请至少分配一名速通者。";
                case TASK_DUEL -> hasRunner
                        ? (hasHunter
                                ? "§c游戏无法开始：多人任务大师模式需要为每个共享身体分配至少一名玩家。"
                                : "§c游戏无法开始：多人任务大师模式需要第二个共享身体。请至少分配一名猎人槽位玩家。")
                        : "§c游戏无法开始：多人任务大师模式需要至少一名速通者。";
                case TASK_RACE -> hasRunner
                        ? (hasHunter
                                ? "§c游戏无法开始：任务竞赛模式只使用速通者。请在开始前移除所有猎人。"
                                : "§c游戏无法开始：任务竞赛模式需要至少两名速通者。")
                        : "§c游戏无法开始：任务竞赛模式需要至少两名速通者。";
            };
            ...
            public void run() {
                if (count > 0) {
                    String title = switch (countdownMode) {
                        case DREAM -> "§b§l追猎交换模式将在 " + count + " 秒后开始";
                        case SAPNAP -> "§d§l速通者交换模式将在 " + count + " 秒后开始";
                        case TASK -> "§6§l任务大师模式将在 " + count + " 秒后开始";
                        case TASK_DUEL -> "§6§l多人任务大师模式将在 " + count + " 秒后开始";
                        case TASK_RACE -> "§6§l任务竞赛模式将在 " + count + " 秒后开始";
                    };
                    String subtitle = "§7由 muj4b 制作";
                    ...
                    String goTitle = switch (countdownMode) {
                        case DREAM -> "§b§l追猎交换模式开始！";
                        case SAPNAP -> "§d§l速通者交换模式开始！";
                        case TASK -> "§6§l任务大师模式开始！";
                        case TASK_DUEL -> "§6§l多人任务大师模式开始！";
                        case TASK_RACE -> "§6§l任务竞赛模式开始！";
                    };
                    for (Player player : getOnlineGameParticipants()) {
                        BukkitCompat.showTitle(player, goTitle, "§7由 muj4b 制作，Flynn汉化", 10, 60, 10);
                    }
                    this.cancel();
                    gameRunning = true;
                    gamePaused = false;
    ...
    public void endTaskCompetitionRound(Player winner, String taskDescription) {
        ...
        String winnerName = winner != null ? winner.getName() : "未知玩家";
        String description = taskDescription != null && !taskDescription.isBlank() ? taskDescription : "他们的秘密任务";

        for (Player participant : getOnlineGameParticipants()) {
            BukkitCompat.showTitle(participant, "§a§l任务完成！",
                    "§e" + winnerName + " §7完成了：§f" + description, 10, 80, 16);
            participant.sendMessage("§a[任务管理器] 获胜者：§f" + winnerName);
        }

        finalizeGameEnd("§a[任务管理器] 获胜者：§f" + winnerName);
    }
    ...
    private String formatEndGameBroadcast(Team winner) {
        String winnerLabel = "无";
        String winnerMessage = "游戏结束！";
        if (winner == Team.RUNNER) {
            winnerLabel = "速通者";
            winnerMessage = "速通者队伍获胜！";
        } else if (winner == Team.HUNTER) {
            winnerLabel = "猎人";
            winnerMessage = "猎人队伍获胜！";
        }
    ...
    private void deliverDonationMessage(Player player, String donateUrl) {
        if (player == null)
            return;
        player.sendMessage("");
        player.sendMessage("§6§l=== 支持创作者 ===");
        player.sendMessage("§e喜欢这个游戏吗？帮助保持更新！");
        player.sendMessage("§d❤ 捐赠支持开发");
        ...
        if (gameRunning && teamEmpty) {
            ...
            if (pauseOnDc) {
                ...
                if (plugin.getConfigManager().isBroadcastGameEvents()) {
                    Msg.broadcast("§e[速通者交换] 游戏已暂停：等待玩家返回。");
                }
            } else {
        ...
        if (gameRunning && plugin.isTaskCompetitionMode()
            ...
            for (Player participant : getTaskCompetitionParticipants())
                if (participant.isOnline())
                    online++;
            if (online <= 1) {
                Msg.broadcast("§e[任务管理器] 游戏结束：仅剩一名参与者。");
                stopGame();
                return;
            }
        }
    ...
    public void handlePlayerJoin(Player player) {
        ...
        if (plugin.isTaskCompetitionMode()) {
            ...
            if (tmm != null && tmm.getAssignedTask(player) != null) {
                ...
                if (def != null) {
                    player.sendMessage("§6[任务管理器] 你的任务：");
    ...
    private void scheduleTaskMaxDurationWatcher(long durationMs) {
        ...
        taskMaxDurationTask = Bukkit.getScheduler().runTaskLater(plugin, () -> {
            ...
            Msg.broadcast("§e[任务管理器] 游戏最大持续时间已到。结束本轮游戏。");
            stopGame();
        }, delayTicks);
    }
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            if (show) {
                ...
                if (isCaged && ((isRunner && !isActive) || (isHunter && plugin.usesSharedSecondBody() && !isActiveHunter))) {
                    ...
                    String msg = String.format("§6队列中 (%d) - 你是下一个", queuePosition);
                    ...
                    String label = isHunter && plugin.usesSharedSecondBody() ? "第二具身体" : "交换";
                    String msg = String.format("§e%s：§c%d秒", label, Math.max(0, displayTime));
                    com.example.speedrunnerswap.utils.ActionBarUtil.sendActionBar(player, msg);
                }
            } else {
                com.example.speedrunnerswap.utils.ActionBarUtil.sendActionBar(player, "");
    ...
    private boolean deferSwapForPortal(Player runner) {
        ...
        if (plugin.getConfigManager().isBroadcastGameEvents()) {
            Msg.broadcast("§e[速通者交换] 交换已延迟：速通者正在穿越传送门。将在 "
    ...
    private void applyRandomPowerUp(Player player) {
        ...
        player.sendMessage(String.format("§%s你获得了一个%s增益效果：%s %s！",
                isGoodEffect ? "a" : "c",
                isGoodEffect ? "正面" : "负面",
                effectName,
                effectLevel));
    ...
    public String getAssignmentRestrictionReason(Player target, Team team, World referenceWorld) {
        ...
        if (team == Team.HUNTER && !huntersAllowed(plugin.getCurrentMode())) {
            return "第二具身体只能在追猎交换模式或多人任务大师模式中分配。";
        }
        if (!plugin.getConfigManager().isMultiworldCompatibilityEnabled()
        ...
        if (!requiredWorld.equals(targetWorld)) {
            return "分配限制在世界 '" + requiredWorld.getName()
                    + "'。" + target.getName() + " 当前在 '" + targetWorld.getName() + "'。";
        }
        return null;
    }
    ...
    private void updateTitles() {
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            ...
            String label = runnerGroup ? "交换倒计时" : "第二具身体";

            // For caged players, show large aesthetic title
            if (isCaged && !isActive) {
                String t = String.format("§6§l%s：%d秒", label, Math.max(0, displayTime));
                String sub = String.format("§e潜行：%s  §7|  §e疾跑：%s", isSneak ? "是" : "否",
                        isSprint ? "是" : "否");
